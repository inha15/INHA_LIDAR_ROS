...
public:
    ...
    void extract(std::vector<pcl::PointIndices>& cluster_indices) {
        ...
        for (int i = 0; i < input_cloud_->points.size(); i++) {
            ...
            int nn_size = radiusSearch(i, eps_, nn_indices, nn_distances); //return point size in eps radius
            if (nn_size < minPts_) { //nn_size = point_num in eps radius
                is_noise[i] = true;
                continue;
            }
            
            //under process point is core point 
            std::vector<int> seed_queue;
            seed_queue.push_back(i);
            types[i] = PROCESSED;
            
            for (int j = 0; j < nn_size; j++) { //it is putting the adjoint point to queue
                if (nn_indices[j] != i) {
            ...
            //there is a clustering obj including whole adjoint point in queue
...
protected:
    ...
    virtual int radiusSearch( //k_indices is idx num, k_sqr_distances is dist as basepoint
        int index, double radius, std::vector<int> &k_indices,
        std::vector<float> &k_sqr_distances) const
    {
        k_indices.clear();
        k_sqr_distances.clear();
        k_indices.push_back(index); //itself include
        k_sqr_distances.push_back(0); //itself dist
        int size = input_cloud_->points.size(); //whole point search
