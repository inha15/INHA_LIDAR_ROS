...
void preClustering_process(const sensor_msgs::PointCloud2ConstPtr& aft_ROI){
    ...
    if( switch_pre_Clustering ) Clustering(prefilteredCloud,TotalCloud,"pre");
...
int main(int argc, char** argv){
    ...
    nh.getParam("/preClustering_node/clustering_offset", clustering_offset);
    nh.getParam("/preClustering_node/MinClusterSize", MinClusterSize);
    nh.getParam("/preClustering_node/MaxClusterSize", MaxClusterSize);
